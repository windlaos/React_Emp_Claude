...
| 우선순위 | 기능 | 상태 |
|---------|------|------|
| 높음 | 폼 유효성 검사 강화 (이메일 형식, 중복 이메일 등) | ✅ 완료 |
| 높음 | 토스트 알림 (등록/수정/삭제 성공 메시지) | ✅ 완료 (react-hot-toast 2.6.0) |
| 중간 | 직원 목록 부서별 필터링 | 미구현 |
| 중간 | 직원 이메일 검색 기능 (`/api/employees/email/:email` 활용) | 미구현 |
| 낮음 | 다크 모드 지원 | 미구현 |
| 낮음 | 페이지네이션 | 미구현 |

### 폼 유효성 검사 구현 상세 (2026-06-02)
- **DepartmentForm**: 부서명 필수·2자 이상·50자 이하 / 에러 시 빨간 테두리 + 인라인 메시지
- **EmployeeForm**:
  - 이름·성: 필수
  - 이메일: 필수 + 정규식 형식 검사 + `/api/employees/email/:email` 중복 체크 (비동기)
  - 수정 모드: `currentEmployeeId` 비교로 본인 이메일 허용
  - 부서: 필수 선택
  - 비동기 검증 중 버튼 비활성화 ("확인 중...")
- 입력 시 해당 필드 에러 즉시 해제 (사용자 피드백 개선)
- Playwright 자동화 검증: 12개 체크 전원 통과

### 토스트 알림 구현 상세 (2026-06-02)
- 라이브러리: `react-hot-toast@2.6.0`
- `<Toaster position="top-right" />` → `Layout.jsx`에 전역 마운트
- 성공 토스트: 부서 등록/수정/삭제, 직원 등록/수정/삭제 (6개 시나리오)
- 실패 토스트: 각 뮤테이션 try-catch에서 `toast.error()` 호출
- Playwright 자동화 검증: 6개 체크 전원 통과
